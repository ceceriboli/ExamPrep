...
def runstest_acfpacf_sw(data, measure):
    """
    ...
    results['runs_test_pval'] = pval_runs
    print('Runs test p-value = {:.3f}'.format(pval_runs))
    if pval_runs < 0.05:
        print("The data are not random (p-value < 0.05).")
    else:
        print("The data are random (p-value >= 0.05).")
    ...
    results['shapiro_wilk_pval'] = pval_SW
    print('Shapiro-Wilk test p-value = {:.3f}'.format(pval_SW))
    if pval_SW < 0.05:
        print("The data are not normally distributed (p-value < 0.05).")
    else:
        print("The data are normally distributed (p-value >= 0.05).")

    # Q-Q plot
    fig_qq = sm.qqplot(data[measure], line='s')
    plt.title('Q-Q Plot')
    plt.show()
